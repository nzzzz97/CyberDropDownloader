...
class HistoryTable:
    ...
    async def check_complete(self, domain: str, url: URL, referer: URL) -> bool:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("""SELECT referer, completed FROM media WHERE domain = %s and url_path = %s""", (domain, url_path))
    ...
    async def check_album(self, domain: str, album_id: str) -> bool | dict[Any, Any]:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("""SELECT url_path, completed FROM media WHERE domain = %s and album_id = %s""", (domain, album_id))
    ...
    async def check_complete_by_referer(self, domain: str, referer: URL) -> bool:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                result = await cursor.execute("""SELECT completed FROM media WHERE domain = %s and referer = %s""", (domain, str(referer)))
                sql_file_check = await cursor.fetchone()
    ...
    async def mark_complete(self, domain: str, media_item: MediaItem) -> None:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("""UPDATE media SET completed = 1, completed_at = CURRENT_TIMESTAMP WHERE domain = %s and url_path = %s""",
                                           (domain, url_path))
    ...
    async def check_filename_exists(self, filename: str) -> bool:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("""SELECT EXISTS(SELECT 1 FROM media WHERE download_filename = %s)""", (filename,))
                sql_file_check = await cursor.fetchone()
    ...
    async def get_downloaded_filename(self, domain: str, media_item: MediaItem) -> str:
        ...
        async with self.db_conn.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("""SELECT download_filename FROM media WHERE domain = %s and url_path = %s""",
                                          (domain, url_path))
                sql_file_check = await cursor.fetchone()
